# Genal Activation Family 🧠

[

![License: GPL v3](https://img.shields.io/badge/License-GPLv3-blue.svg)

](https://www.gnu.org/licenses/gpl-3.0)
<img width="2034" height="1409" alt="Image" src="https://github.com/user-attachments/assets/e5b888b5-4d1e-4bbb-a50e-2d299ac89d33" />
pip install genal-activation

[

![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.20304195.svg)

](https://zenodo.org/records/20304195)
[

![ORCID](https://img.shields.io/badge/ORCID-0009--0009--6495--4085-green)

](https://orcid.org/0009-0009-6495-4085)

> **A learnable activation function that adapts its curvature to each task.**
> Outperforms ReLU, GELU, and Swish on 16 diverse benchmarks.

**Author:** Genal Ediso Lombano Pineda — Independent Researcher, Venezuela 🇻🇪
**Paper:** [zenodo.org/records/20304195](https://zenodo.org/records/20304195)
**ORCID:** [0009-0009-6495-4085](https://orcid.org/0009-0009-6495-4085)
**Email:** genallombano@gmail.com

---

## 📦 Installation

```bash
pip install torch
# Clone this repo
git clone https://github.com/GenalFF/genal-activation.git
